...
public class BSTD<T extends Comparable<T>>{
    ...
    */
    private void insert(T key, BTDNode<T> stRoot) {
        ...
        if(isEmpty()) {
            ...
            //BTDNode<T> temp = root;
        	if(key.compareTo(stRoot.getKey()) == 0){
        		stRoot.addKey(key);
        }else if(key.compareTo(stRoot.getKey()) < 0) {
    ...
    */
    private void inOrderPrint(BTDNode<T> stRoot) {
        if(stRoot != null) {
            ...
            //
            LinkedList<T> list = stRoot.getKeyList();
    ...
    */
    ...
    /**
     * Method that prints the values in a tree within a certain range
     */
    public void printInRange(T low, T high){
  
    		printInRange(low, high, root);
    	
    }
    
    
    /**
     * Recursive method that only prints out values within a certain range
     */
    private void printInRange(T low, T high, BTDNode<T> stRoot){
        if(stRoot != null) {
            // Recursively call method to print the left sub-tree
            printInRange(low, high, stRoot.getLeft());
            
            //
    
            if(stRoot.getKey().compareTo(low) >= 0 && stRoot.getKey().compareTo(high) <= 0){
                LinkedList<T> list = stRoot.getKeyList();
	            for(int i = 0; i < list.size(); i++){
	            	System.out.println(list.get(i));
            }
            ...
            // Recursively call method to print the right sub-tree
            printInRange(low, high, stRoot.getRight());
        }
